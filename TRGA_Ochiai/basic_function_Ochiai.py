...
def generate_rank(meature_metric,statements_number):
    ...
    for i in range(1, statements_number + 1):
        if (meature_metric[i - 1][3] == 0):
            ...
            tmp_ochiai = (meature_metric[i - 1][3] + meature_metric[i - 1][1]) * (meature_metric[i - 1][3] + meature_metric[i - 1][2])
            tmp_ochiai = math.sqrt(tmp_ochiai)
            suspect_tarantula = meature_metric[i - 1][3] / (tmp_ochiai)
