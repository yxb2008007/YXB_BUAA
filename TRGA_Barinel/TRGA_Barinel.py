...
import xlrd,xlwt,basic_function_Barinel,random,Gene_function_standard
...
for version in version_list:
    ...
    measure_ori = basic_function_Barinel.generate_initial_measure(matrix_sample, execution_result_sample,num_testcases, num_statements)
    rank_ori = basic_function_Barinel.generate_rank(measure_ori, num_statements)
    expense_ori = basic_function_Barinel.get_expense_score(faulty_statements_list, rank_ori)
    ...
    if(len(list_fail)==1):
        ...
        measure_conversion = basic_function_Barinel.generate_mesure_conversion_all(matrix_sample, measure_ori,
                                                                                   list_set, num_statements)
        rank_conversion = basic_function_Barinel.generate_rank(measure_conversion, num_statements)
        expense_conversion = basic_function_Barinel.get_expense_score(faulty_statements_list, rank_conversion)
